	...
	try {
		...
		if (test.questions_to_ask && test.questions_to_ask > 0) {
			...
			if (test.questions_to_ask < allQuestionIds.length) {
				...
				console.log(
					`🎲 Randomly selected ${test.questions_to_ask} questions out of ${allQuestionIds.length}`,
				);
			}
		}
		...
		if (existingAttempt.rows.length > 0) {
			console.log(
				"⚠️ In-progress attempt already exists:",
    ...
    `,
		...
		if (error.code === "23505") {
			return res.status(409).json({
				...
				details:
					"Please continue your existing attempt or contact administrator",
			});
		}

		res
			.status(500)
			.json({ error: "Failed to start test", details: error.message });
	}
};
...
exports.submitAnswer = async (req, res) => {
	try {
		const { attempt_id, question_id, answer, code_submission, is_flagged } =
			req.body;
...
exports.submitAttempt = async (req, res) => {
	try {
		...
		const attemptResult = await db.query(
			"SELECT test_id, selected_questions FROM test_attempts WHERE id = $1",
			[attempt_id],
		);
		...
		if (attempt.selected_questions && attempt.selected_questions.length > 0) {
			...
			const placeholders = attempt.selected_questions
				.map((_, i) => `$${i + 1}`)
				.join(",");
    ...
    `,
		...
		const testResult = await db.query("SELECT title FROM tests WHERE id = $1", [
			attempt.test_id,
		]);
		const testTitle = testResult.rows[0]?.title || "Test";
		...
		try {
			await createNotification(
				...
				"grade_update",
				"Test Submitted",
				`Your test "${testTitle}" has been submitted. Score: ${score}/${totalPoints} (${Math.round(percentage)}%)`,
				{
		...
		// Send email via MCP tool (do not fail submission if email fails)
		try {
			const studentName = req.user?.name || "Student";
			const studentEmail = req.user?.email;
			if (studentEmail) {
				const subject = `Test submitted: ${testTitle}`;
				const text =
					`Hi ${studentName},\n\n` +
					`Your test "${testTitle}" has been submitted successfully.\n\n` +
					`Score: ${score}/${totalPoints} (${Math.round(percentage)}%)\n` +
		...
		// Auto-issue certificate if student qualifies (non-blocking)
		try {
			const certificateService = require("../services/certificateService");
			const certResult =
				await certificateService.autoIssueCertificate(attempt_id);
			if (certResult.issued) {
				console.log(`Certificate auto-issued for attempt ${attempt_id}`);
			}
		} catch (certError) {
...
exports.getAttemptReview = async (req, res) => {
	try {
      ...
      `,
			...
			// Teachers/Admins can view attempt reviews, but teachers must own the test.
			attemptResult = await db.query(
				`
        SELECT ta.*, t.title as test_title, t.passing_score, t.show_review_to_students, t.created_by
        FROM test_attempts ta
        JOIN tests t ON ta.test_id = t.id
        WHERE ta.id = $1
      `,
		...
		if (req.user.role === "student" && !attemptData.show_review_to_students) {
			return res
				.status(403)
				.json({ error: "Review is not available for this test" });
		}

		// If teacher, verify they are the test owner
		if (req.user.role === "teacher") {
			if (attemptData.created_by !== req.user.id) {
				return res.status(403).json({
					error:
						"Access denied. You can only view reviews for tests you created.",
				});
			}
		}
    ...
    `,
		...
		const processedAnswers = answersResult.rows.map((answer) => ({
			...
			test_cases: answer.test_cases
				? typeof answer.test_cases === "string"
					? JSON.parse(answer.test_cases)
					: answer.test_cases
				: null,
			test_results: answer.test_results
				? typeof answer.test_results === "string"
		...
		let totalPoints = 0;
		if (
			attemptData.selected_questions &&
			attemptData.selected_questions.length > 0
		) {
...
exports.getLiveAttempts = async (req, res) => {
    ...
    `,
		...
		if (req.user.role === "teacher" && test.created_by !== req.user.id) {
			...
			return res.status(403).json({
				error:
					"Access denied. You can only view attempters for tests you created.",
			});
		}
...
exports.getAccessibleTests = async (req, res) => {
	try {
      ...
      `,
		...
		console.log(
			`✅ Found ${testsResult.rows.length} accessible tests for ${userRole}`,
		);
...
exports.getTestReport = async (req, res) => {
    ...
    `,
		...
		if (userRole === "teacher" && test.created_by !== userId) {
			...
			return res.status(403).json({
				error:
					"Access denied. You can only view reports for tests you created.",
			});
		}
    ...
    `,
		...
		const statistics = {
			...
			in_progress_count: attempts.filter((a) => a.status === "in_progress")
				.length,
			average_score:
				scores.length > 0
			...
			pass_count: submittedAttempts.filter(
				(a) => parseFloat(a.score) >= test.passing_score,
			).length,
			fail_count: submittedAttempts.filter(
				(a) => parseFloat(a.score) < test.passing_score,
			).length,
			total_violations: attempts.reduce(
				(sum, a) => sum + (parseInt(a.violation_count) || 0),
				0,
			),
		};
		...
		res.json({
			test: {
				...test,
				question_count: await db
					.query("SELECT COUNT(*) FROM questions WHERE test_id = $1", [testId])
...
exports.getStudentAttempts = async (req, res) => {
	try {
		...
		if (
			...
			studentId !== requesting_user_id
		) {
			return res
				.status(403)
				.json({ error: "You can only view your own test attempts" });
		}
    ...
    `,
		...
		res.json({
			...
			best_score:
				attemptsResult.rows.length > 0
					? Math.max(
							...attemptsResult.rows.map((a) => parseFloat(a.percentage) || 0),
						)
					: 0,
		});
	} catch (error) {
    ...
    `,
      ...
      `,
			...
			if (test.created_by !== requesting_user_id) {
				return res.status(403).json({
					error:
						"Access denied. You can only view attempts for tests you created.",
				});
			}
		}
