...
export default function LiveAttempts({
	...
	if (accessDenied) {
		return (
			<View className="flex-1 p-6 items-center justify-center">
				...
				<Text
					className={`mt-4 text-xl font-bold ${isDark ? "text-white" : "text-gray-900"}`}
				>
					Access Denied
				</Text>
				<Text
	...
	return (
		<View className="flex-1">
			{/* Header Stats */}
			<View
				className={`p-4 border-b ${isDark ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"}`}
			>
				<View className="flex-row justify-between items-center mb-3">
					...
					<View className="flex-row items-center gap-2">
						<View
							className={`w-2 h-2 rounded-full ${liveCount > 0 ? "bg-green-500" : "bg-gray-400"}`}
				...
				<View className="flex-row gap-4">
					<View
						className={`flex-1 p-3 rounded-lg ${isDark ? "bg-green-900/30" : "bg-green-50"}`}
					>
					...
					<View
						className={`flex-1 p-3 rounded-lg ${isDark ? "bg-blue-900/30" : "bg-blue-50"}`}
					>
						<View className="flex-row items-center gap-2 mb-1">
					...
					<View
						className={`flex-1 p-3 rounded-lg ${isDark ? "bg-purple-900/30" : "bg-purple-50"}`}
					>
						<View className="flex-row items-center gap-2 mb-1">
						...
						{attempts.map((attempt) => (
							...
							>
									...
									<View className="mb-3">
										...
										>
											<View
												...
												style={{
													width: `${getProgressPercentage(attempt.answered_count, attempt.total_questions)}%`,
												}}
											/>
										</View>
									</View>
								...
								{attempt.status === "submitted" && (
									<View
										className={`mb-3 p-3 rounded-lg ${isDark ? "bg-gray-900/50" : "bg-gray-50"}`}
									>
								...
								{attempt.status === "in_progress" && (
									<View className="mt-3 pt-3 border-t border-gray-600">
										<Text
											className={`text-xs text-center ${isDark ? "text-green-400" : "text-green-600"}`}
										>
